...
/* ────────────────────────────────
   CONTACT  ← EDGE FUNCTION INTEGRATED
──────────────────────────────── */
const EMPTY_FORM = { name: "", business: "", email: "", phone: "", message: "", website: "" };
            ...
            {status === "success" ? (
              ...
              <form onSubmit={handleSubmit} style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", padding: "48px 40px", display: "flex", flexDirection: "column", gap: 20 }}>
                ...
                <div><label style={labelStyle}>Your name</label>
                  <input type="text" placeholder="Jane Smith" required maxLength={FIELD_LIMITS.name} value={form.name} onChange={handleChange("name")} style={inputStyle} onFocus={onFocus} onBlur={onBlur} />
                </div>

                <div><label style={labelStyle}>Business name</label>
                  <input type="text" placeholder="Smith & Co Plumbing" required maxLength={FIELD_LIMITS.business} value={form.business} onChange={handleChange("business")} style={inputStyle} onFocus={onFocus} onBlur={onBlur} />
                </div>

                <div><label style={labelStyle}>Email address</label>
                  <input type="email" placeholder="jane@smithplumbing.co.za" required maxLength={FIELD_LIMITS.email} value={form.email} onChange={handleChange("email")} style={inputStyle} onFocus={onFocus} onBlur={onBlur} />
                </div>

                <div><label style={labelStyle}>Phone number</label>
                  <input type="tel" placeholder="+27 82 000 0000" required maxLength={FIELD_LIMITS.phone} value={form.phone} onChange={handleChange("phone")} style={inputStyle} onFocus={onFocus} onBlur={onBlur} />
                </div>

                <div><label style={labelStyle}>Anything else we should know? (optional)</label>
                  <textarea rows={3} placeholder="Tell us a bit about your business..." maxLength={FIELD_LIMITS.message} value={form.message} onChange={handleChange("message")} style={{ ...inputStyle, resize: "vertical" }} onFocus={onFocus} onBlur={onBlur} />
                </div>
